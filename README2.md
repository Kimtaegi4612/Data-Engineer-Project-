...
```
       ...
       |  PostgreSQL 15   |
       |                  |
       |  Bronze Layer    | -- Transform -->  |  Silver Layer   | -- Aggregate -->  |  Gold Layer   |
       |  ticker_raw      |                   |  ticker         |                   |  market_stats |
       +--------+---------+                   +-----------------+                   +-------+-------+
                |                                                                           |
                +---------------------------------------------------------------------------+
                                                                                            |
...
### 메달리온 아키텍처 상세 (Medallion Architecture Detail)

데이터가 **Bronze → Silver → Gold** 로 흐르며 점진적으로 정제·집계되는 구조입니다.

```
  +-- Bronze Layer (Raw) -------------------------------------------------+
  |  Upbit API 호출  -->  API 응답 JSON  -->  bronze.ticker_raw           |
  |  raw_data (JSONB), market, ingested_at / 가공 없이 원본 보존          |
  +----------------------------------------+-----------------------------+
                                           | ELT Transform
  +----------------------------------------v-----------------------------+
  |  Silver Layer (Cleansed)                                              |
  |  JSON 파싱 & 스키마 표준화  -->  타입 변환, 결측/이상치  -->  silver.ticker |
  |  market, trade_date, trade_time, trade_price, acc_trade_volume (정형)  |
  +----------------------------------------+-----------------------------+
                                           | Aggregate
  +----------------------------------------v-----------------------------+
  |  Gold Layer (Data Mart)                                                |
  |  KPI 집계, 윈도우 함수  -->  RANK(), 집계 함수  -->  gold.market_stats  |
  |  market, trade_price, daily_volume, rank_volume / 대시보드 조회 최적화 |
  +------------------------------------------------------------------------+
```
...
| 레이어 | 테이블 | 입력 | 출력 | 주요 연산 |
|--------|--------|------|------|-----------|
| **Bronze** | `bronze.ticker_raw` | API JSON | JSONB 적재 | 수집만, 스키마 유연 |
| **Silver** | `silver.ticker` | JSONB | 정형 테이블 | 파싱, 타입 캐스팅, 결측 처리 |
| **Gold** | `gold.market_stats` | Silver 테이블 | KPI 마트 | RANK(), SUM, 집계 |
...
```
  ...
  collect_ticker     -->  transform_to_silver     -->  aggregate_to_gold
  (Bronze 적재)            (Silver 적재)                 (Gold 적재)
       |                         |                            |
       | BashOperator             | BashOperator               | BashOperator
       v                         v                            v
  src/collect_*.py         src/transform_*.py           src/aggregate_*.py
```
...
```
  Bronze   API JSON  -->  raw_data (JSONB)  -->  원본 보존, 불변성
       |
       v
  Silver   JSON 파싱  -->  타입/결측 처리  -->  정형 테이블 (silver.ticker)
       |
       v
  Gold     집계, RANK()  -->  KPI 마트  -->  gold.market_stats --> 대시보드
```
...
```
  Upbit API  -->  Bronze (ticker_raw, JSONB)  -->  Silver (ticker)  -->  Gold (market_stats, KPI)  -->  Metabase
                   원본 저장                      정형 변환              KPI 집계                    대시보드
```
